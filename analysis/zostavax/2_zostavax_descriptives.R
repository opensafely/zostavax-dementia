...
check_discontinuity_pre <- function(
  .data,
  dob_threshold_date,
  index_date,
  event_col,
  ...
  plot_summary <-
    ...
    scale_x_date(
      ...
      name = "Date of birth",
      labels = ~ scales::label_date("%d %b %y")(.),
      breaks = dob_threshold_date + months(seq(-10, 10) * 12),
  ...
  ggsave(
    plot_summary,
    filename = glue("discontinuity_pre_{event_name}.png"),
    path = output_dir
  )
}

check_discontinuity_pre(
  df_analysis,
  dob_threshold_date,
  index_date,
...
check_discontinuity_pre(
  ...
  dob_threshold_date,
  index_date,
  "cognitive_impair_before_threshold",
...
check_discontinuity_pre(
  ...
  dob_threshold_date,
  index_date,
  "antihypertensives_before_threshold",
...
cumulative_events <- function(
  ...
  write_csv(
    ...
    fs::path(
      output_dir,
      glue("cumulative_incidence_{event_name}_{time_horizon}.csv")
    )
  )
  ...
  plot_km <-
    ...
    geom_rect(
      aes(xmin = lagtime, xmax = time, ymin = cmlinc.low, ymax = cmlinc.high),
      alpha = 0.1,
      colour = "transparent"
    ) +
  ...
  ggsave(
    plot_km,
    filename = glue("cumulative_incidence_{event_name}_{time_horizon}.png"),
    path = output_dir,
    width = 15,
    height = 15,
    units = "cm"
...
cumulative_events(
  df_analysis |>
    mutate(
      month_of_birth = scales::label_date(format = "%y %b")(month_of_birth)
    ) |>
    filter(between(age, 79, 80)),
  "month_of_birth",
  precision = 7,
  365,
  "zostavax_date_1",
...
cumulative_events(
  ...
  precision = 7,
  365,
  "varicella_first_date_after",
  "Varicella vaccine"
)

cumulative_events(
  df_analysis |> filter(!asthma_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!afib_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!chd_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!ckd_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!copd_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!depression_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!t2dm_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!epilepsy_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!hf_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!hypothyroid_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!osteoporosis_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!pad_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!ra_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!smi_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
cumulative_events(
  df_analysis |> filter(!obese_before_threshold),
  "eligibility",
  precision = 7,
  365,
...
check_discontinuity_post <- function(
  .data,
  dob_threshold_date,
  index_date,
  time_horizon,
  ...
  dat_summary <-
    ...
    transmute(
      ...
      event_time = as.integer(
        pmin(event_date, censor_date, index_date + time_horizon, na.rm = TRUE) -
          index_date
      ),
  ...
  ggsave(
    plot_summary,
    filename = glue("discontinuity_{event_name}_{time_horizon}.png"),
    path = output_dir
  )
}
