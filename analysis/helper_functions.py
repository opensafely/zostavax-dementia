...
def first_vax_event_after(reference_date, product_name=None, target_disease=None):
    ...
    if product_name is not None:
        return(
            ...
            .where(vaccinations.date.is_on_or_after(reference_date))
...
def last_vax_event_before(reference_date, product_name=None, target_disease=None):
        
    if product_name is not None:
        return(
            vaccinations
            .where(vaccinations.product_name.is_in(product_name))
            .where(vaccinations.date.is_before(reference_date))
            .sort_by(vaccinations.date)
            .last_for_patient()
        )
...
def first_gp_event_after(reference_date, codelist):
    
    return(
        clinical_events
        .where(clinical_events.snomedct_code.is_in(codelist))
        .where(clinical_events.date.is_on_or_after(reference_date))
        .sort_by(clinical_events.date)
        .first_for_patient()
    )

def last_gp_event_before(reference_date, codelist):

    return(
        clinical_events
        .where(clinical_events.snomedct_code.is_in(codelist))
        .where(clinical_events.date.is_before(reference_date))
        .sort_by(clinical_events.date)
        .last_for_patient()
    )
...
def last_rx_event_before(reference_date, codelist):
        
    return(  
        medications
        .where(medications.dmd_code.is_in(codelist))
        .where(medications.date.is_before(reference_date))
...
def last_hosp_event_before(reference_date, codelist):
